                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="content">
                                    ...
                                    @endif
                                    <form class="row g-3" id="product-detail" method="post" action="{{ route('admin.product.update') }}">
                                        @csrf
                                        <input type="hidden" name="product_id" value="@if(isset($product)){{ $product['id'] }}@endif">
                                        <div class="col-md-12">
                                            <label for="title" class="form-label">Tên sản phẩm</label>
                                            <input type="text" class="form-control" name="title" id="title" value="@if(isset($product)){{ $product['title'] }}@endif" required>
                                        </div>
                                        <div class="col-12">
                                            <label for="slug" class="form-label">Đường dẫn</label>
                                            <input type="text" class="form-control" id="slug" name="slug" value="@if(isset($product)){{ $product['slug'] }}@endif" required>
                                        </div>
                                        <div class="col-12">
                                            ...
                                            <select class="form-control" name="category_sub">
                                                @foreach($category_sub as $item)
                                                    <option value="{{ $item['id'] }}" @if(isset($product) && $item['id'] == $product['category_sub_id'])selected @endif>{{ $item['name'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12">
                                            <label for="description" class="form-label">Thông tin sản phẩm</label>
                                            <textarea class="form-control" name="description" id="description">@if(isset($product)){{ $product['long_description'] }}@endif</textarea>
                                        </div>
                                        <div class="col-md-6">
                                            <label for="price-old" class="form-label">Giá cũ</label>
                                            <input type="number" class="form-control" name="price_old" id="price-old" value="@if(isset($product)){{ $product['price_old'] }}@endif">
                                        </div>
                                        <div class="col-md-6">
                                            <label for="price" class="form-label">Giá thực tế</label>
                                            <input type="number" class="form-control" name="price" id="price" value="@if(isset($product)){{ $product['price'] }}@endif">
                                        </div>
                                        <div class="col-md-12">
                                            ...
                                            Thêm ảnh : <input type="file" class="btn btn-light add-image" data-id="@if(isset($product)){{ $product['id'] }}@endif" name="images[]" multiple accept="image/*">
                                        </div>
                                        <div class="col-md-12">
                                            <label class="form-label">Biến thể</label>
                                            <select class="form-control" name="variants" id="variants">
                                                @foreach($variants as $item)
                                                    <option value="{{ $item['id'] }}" @if(isset($product) && count($product_variant) && $item['id'] == $product_variant[0]['variant_id'])selected @endif>{{ $item['name'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-12">
